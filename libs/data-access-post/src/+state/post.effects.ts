...
export class PostEffects {
  ...
  addComment$ = createEffect(() =>
    this.actions$.pipe(
      ...
      withLatestFrom(this.store.select(selectPost)),
      switchMap(([{ content, author }, post]) => {
        if (!post) {
          return of(PostActions.addCommentFailure({ error: 'Post not loaded' }));
        }
        return this.postService.addComment(post.id, content, author).pipe(
          map(() => PostActions.addCommentSuccess()),
          catchError(error => of(PostActions.addCommentFailure({ error: error.message ?? String(error) })))
        );
      })
    )
  );
  ...
  notifyAddComment$ = createEffect(
    () =>
      this.actions$.pipe(
        ofType(PostActions.addCommentSuccess),
        tap(() => {
